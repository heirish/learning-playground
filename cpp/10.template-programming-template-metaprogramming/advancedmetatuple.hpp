...
struct Tuple<ELEM0, ELEM1ToN...> : Tuple<ELEM1ToN...> {
    ...
    // Tuple{1,c,false} will not work because c is an lvalue. (int&& is an
    // rvalue reference, while T&& is universal reference) to implement a
    // forwarding reference, there has to be a type deduction involved.. need to
    // make constructor as template.
    template <typename T, typename... Ts>
    // 这里T,
    // Ts都忘记加&&了，std::forward会导致其move,因此stats输出会多两个.moves
...
template <typename TUPLE> struct get_impl<0, TUPLE> {
    ...
    template <typename T> constexpr static decltype(auto) get(T &&t) {
        ...
        // constexpr bool is_const = std::is_const_v<T>;
        // becarefull, as a forward reference, it's always deduced to a
        // reference type, we need to strip off this reference before we
        // composite it to is_const_v, otherwise it won't work correctly
        constexpr bool is_const = std::is_const_v<std::remove_reference_t<T>>;
        using data_t = meta_typelist_lib::front_t<TUPLE>;
        if constexpr (is_const &&
                      is_lvalue) { // to make it work for const Tuple also
            return static_cast<const data_t &>(
                static_cast<const TUPLE &>(t).data);
        } else if constexpr (!is_const && is_lvalue) {
